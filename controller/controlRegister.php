...
class controller_Register {
   ...
   public function simpandata(){
	...
	public function resetpassword(){
		...
		$setting		= $modelSetting->getSettingToko();
	   ...
	   $customer = $this->dataModel->getCustomerByEmail($this->data['email']);
	   
	   if($customer && $this->data['sesicapca'] == $this->data['capcaku']) {
			...
			$this->data['id']		= $customer['cust_id'];
			$this->dataModel->simpanAktivitas($this->data);
		  
			/* kirim password ke remail */
		  
			$from = $config_email; //alamat toko
			$from_name = $config_namatoko;
			...
			$bodys    .= '<div style="font-family: Arial, Helvetica, sans-serif; font-size: 12px;">
						<br><br>
					-------------------------------------------------------------------------------------<br>
					JANGAN BALAS EMAIL NOTIFIKASI INI. <br>
					-------------------------------------------------------------------------------------<br>
						<br>';
			$bodys    .= "Anda telah melakukan Reset Password Login di Toko kami<br><br>";
			$bodys	.= "Email : ".$this->data['email']."<br>";
			$bodys	.= "Password : ".$password."<br>";
			$bodys	.= '<a href="'.$config_alamatsite.'/login">Klik disini login</a><br><br>';
			$bodys	.= '<br><br>Selamat berbelanja di Toko '.$config_namatoko.'<br>';
			...
			$this->kirim_remail->MsgHTML($bodys);
			$this->kirim_remail->CharSet="UTF-8";
			$this->kirim_remail->AddAddress($to,$this->data['email']);
			$this->kirim_remail->Send();
			/***************************/
			$status = 'success';
			$pesan = 'Reset Password telah dikirim ke email Anda';
		} else {
			$status = 'error';
			$pesan = 'Perhatikan Kode Sekurity';
		}
