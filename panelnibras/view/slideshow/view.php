   ...
   <div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['id_slide']?>" /></td>
		   <td>
		   
		   <?php echo $datanya["nama_slide"]?> (<?php echo $datanya["panjang"]?> x <?php echo $datanya["lebar"]?>)<br>
           <?php if($datanya['gbr_slide'] != '') { ?>
		   <img src="<?php echo URL_IMAGE.'_other/other_'.$datanya["gbr_slide"]?>" class="img-responsive" style="max-width:30%">
		 
		   <?php } ?>
		   </td>
		   <td><?php echo $datanya['urutan']=='' || $datanya['urutan'] == Null  ? '0' : $datanya['urutan'] ?></td>
		   <td><?php echo $datanya['sts_slide']=='1' ? 'Ya':'Tidak' ?></td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['id_slide']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
