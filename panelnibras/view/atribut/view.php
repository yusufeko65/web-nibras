		...
		<div class="table">
		    <table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr <?php echo $kelasgrid ?>>
				<td><input type="checkbox" class="chk" value="<?php echo $datanya['id_atribut']?>" /></td>
				<td><?php echo $datanya["nama_atribut"]?></td>
				<td><?php echo $datanya["nama_atribut_grup"]?></td>
				<td style="display:none"><?php if($datanya["value"] != '-') { ?>
				            <div style="margin:auto;border:1px solid #000;height:15px;width:15px;background:#<?php echo $datanya["value"] ?>"></div>
				    <?php } ?>
				</td>
				<td><a class="ico edit" onclick="tampilkan('<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['id_atribut']?>')">Edit</a></td>
			  </tr>
			  <?php } ?>
			</table>
			<?php if($total>0) { ?>
