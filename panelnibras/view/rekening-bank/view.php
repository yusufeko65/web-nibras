   ...
   <div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['rekening_id']?>" /></td>
		   <td><?php echo $datanya["rekening_no"]?></td>
		   <td><?php echo $datanya["bank_nama"]?></td>
		   <td><?php echo $datanya["rekening_cabang"]?></td>
		   <td><?php echo $datanya["rekening_atasnama"]?></td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['rekening_id']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
