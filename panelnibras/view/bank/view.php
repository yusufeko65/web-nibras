   ...
   <div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['bank_id']?>" /></td>
		   <td><?php echo $datanya["bank_nama"]?><?php if($datanya['bank_logo'] != '') { ?><br><img src="<?php echo URL_IMAGE.'_other/other_'.$datanya["bank_logo"]?>"><?php } ?></td>
		   <td><?php echo $datanya['bank_status']=='1' ?'Ya' : 'Tidak';?></td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['bank_id']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
