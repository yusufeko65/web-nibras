...
function simpandata(){ //Proses Simpan
	...
	if($.trim(komentar) == ''){
		$('#komentar').focus();
		$('#hasil').html("<div class=\"alert alert-danger\"> Masukkan Komentar </div>");
		$('#hasil').show(500);
		return false;
	}
	
	datasimpan = "aksi="+aksi+"&nama="+nama+"&email="+email+"&urlweb="+urlweb+"&komentar="+komentar+"&status="+status+"&iddata="+iddata;
	$('#loadingweb').show(500);
	$.ajax({
 		type: "POST",
   		url: action,
