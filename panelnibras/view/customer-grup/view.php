	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  <thead>
		 <tr>
		   ...
		   <td>Nama Grup</td>
		   <td style="min-width:15%" >Potongan</td>
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['cg_id']?>" /></td>
		   <td><?php echo $datanya["cg_nm"]?></td>
		   <td class="text-center"><?php echo $datanya["cg_diskon"]?> %</td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['cg_id']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
