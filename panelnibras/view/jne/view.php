   ...
   <div class="row">
	...
	<table class="table table-bordered table-striped table-hover tabel-grid">
	  ...
	  <tbody id="viewdata">
		...
		<tr>
		   <td class="tengah"><input type="checkbox" class="chk" value="<?php echo $datanya['idt']?>" /></td>
		   <td class="text-right"><?php echo $dtFungsi->fFormatuang($datanya["hrg_perkilo"])?></td>
		   <td class="text-center"><?php echo $datanya["servis_code"]?></td>
		   <!--<td class="text-center"><?php echo $datanya["keterangan"]?> hari</td>-->
		   <td><?php echo $datanya["kecamatan_nama"]?>, <?php echo $datanya["kabupaten_nama"]?>, <?php echo $datanya["provinsi_nama"]?></td>
		   <td class="tengah"><a href="<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit&pid=".$datanya['idt']?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
		</tr>
		<?php } ?>
	   </tbody>
	 </table>
