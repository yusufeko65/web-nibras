...
<div class="kotakplat">
	...
	<div class="body">
	    ...
	    <div class="left">
			...
			<a onclick="tampilkan('<?php echo URL_PROGRAM_ADMIN.folder."/?op=add-diskon"?>')" class="tombols biru">+ Tambah</a>
		</div>
		<div class="right">
		...
		<div class="table">
		    <table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				 ...
				 <th width="50%" class="ac">Nama Diskon</th>
			  ...
			  <tr <?php echo $kelasgrid ?>>
				<td><input type="checkbox" class="chk" value="<?php echo $datanya['idjnedisk']?>" /></td>
				<td class="bariskiri"><?php echo $datanya["nm_diskon"]?></td>
				<td class="bariskanan">
				<?php if($datanya['persen']=='0') { ?>
				<?php echo $dtFungsi->fFormatuang($datanya["jml_disk"])?>
				<?php } else { ?>
				<?php echo $datanya["jml_disk"].'%'?>
				<?php } ?>
				</td>
				<td><a class="ico edit" onclick="tampilkan('<?php echo URL_PROGRAM_ADMIN.folder."/?op=edit-diskon&pid=".$datanya['idjnedisk']?>')">Edit</a></td>
			  </tr>
			  <?php } ?>
			</table>
			<?php if($total>0) { ?>
...
function hapusdata(){
	...
	dataId = ids.join(':');
	datahapus = 'aksi=hapusdiskon&id=' + dataId;
	if(dataId==""){
	   ...
	   if (a == true) {
			$.ajax({
				...
				success: function(msg){
					...
					tampilkan('<?php echo URL_PROGRAM_ADMIN.folder.'/?op=view-diskon' ?>');
					return false;
				},  
					error: function(e){  
					alert('Error: ' + e);  
